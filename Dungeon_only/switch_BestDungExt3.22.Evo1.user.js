// ==UserScript==
// @name			switch_UPBestDungeonExt3
// @name:en			switch_UPBestDungeonExt3
// @name:ru			switch_UPBestDungeonExt3
// @namespace		switch_UPBestDungeonExt3
...
`;
	...
	document.addEventListener('changeDungeonProfile', function (event) {
		...
		// SWITCH: Profile 0 = RED mode, deactivate Evo1
		if (profileNumber === 0) {
			console.log(`%c[AutoProfiler] Profile 0 (RED) received. Deactivating Evo1 engine.`, 'color: #f0a');
			globalThis.__evo1Active = false;
			document.dispatchEvent(new CustomEvent('dungeonProfileState', { detail: { profileNumber: 0 } }));
			return;
		}
		...
		// Activate Evo1 engine on any profile 1+
		globalThis.__evo1Active = true;
	...
	// ==========================================================
	// --- SWITCH: Engine activation listener ---
	document.addEventListener('setDungeonEngine', function (event) {
		const engine = event.detail.engine;
		if (engine === 'red') {
			globalThis.__evo1Active = false;
			console.log(`%c[Switch] Evo1 engine DEACTIVATED by setDungeonEngine (RED).`, 'color: #f0a');
		} else if (engine === 'evo1') {
			globalThis.__evo1Active = true;
			console.log(`%c[Switch] Evo1 engine ACTIVATED by setDungeonEngine.`, 'color: #f0a');
		}
	});
	// ==========================================================
	...
	document.addEventListener('autoStartDungeon', function (event) {
		if (!globalThis.__evo1Active) return;
	...
	// SWITCH: Engine active flag. Default ON (Evo1).
	globalThis.__evo1Active = true;
	const OriginalDungeonFixBattle = HWHClasses.DungeonFixBattle;

	class UpdateDungeonFixBattle extends DungeonFixBattle {
		getTimer() {
			// SWITCH: If Evo1 is not active, defer to original behavior
			if (!globalThis.__evo1Active) {
		...
		setState() {
			// SWITCH: If Evo1 is not active, skip
			if (!globalThis.__evo1Active) return;
	...
	class BestDungeon {
		constructor(resolve, reject) {
			// SWITCH: If Evo1 is deactivated, delegate to RED's original executeDungeon
			if (!globalThis.__evo1Active && globalThis.__originalExecuteDungeon) {
				return new globalThis.__originalExecuteDungeon(resolve, reject);
			}
		...
		async checkFloor(dungeonInfo) {
			...
			// SWITCH: Se Evo1 viene disattivato mentre è in esecuzione, ferma il loop
			if (!globalThis.__evo1Active) {
				this.endDungeon('endDungeon', 'Evo1 deactivated');
				return;
			}
		...
		updatePower(battle) {
			...
			if (buff) { this.buffHealing = buff; unsafeWindow._lastHealingBuff = buff; }
		...
		async endBattle(battleInfo) {
			...
			let resultEnd;
			try {
				resultEnd = await Caller.send({ name: 'dungeonEndBattle', args });
			} catch (err) {
				console.warn('[Switch] dungeonEndBattle failed — re-fetching dungeon info:', err);
				try {
					this.dungeonGetInfo = await Caller.send('dungeonGetInfo');
					this.checkFloor(this.dungeonGetInfo);
				} catch (e) {
					console.warn('[Switch] dungeonGetInfo also failed — ending dungeon:', e);
					this.endDungeon('endDungeon', I18N('STOPPED'));
				}
				return;
			}
		...
		// VERSIONE MODIFICATA
		async resultEndBattle(battleResult) {
			if (battleResult.error) {
				console.warn('[Switch] resultEndBattle error — re-fetching dungeon info:', battleResult.error);
				try {
					this.dungeonGetInfo = await Caller.send('dungeonGetInfo');
					this.checkFloor(this.dungeonGetInfo);
				} catch (e) {
					console.warn('[Switch] dungeonGetInfo failed in resultEndBattle — ending dungeon:', e);
					this.endDungeon('endDungeon', I18N('STOPPED'));
				}
				return;
			}
	...
	// SWITCH: Save RED's original executeDungeon before overriding
	globalThis.__originalExecuteDungeon = this.HWHClasses.executeDungeon;

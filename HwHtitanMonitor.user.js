...
// @version         8.0.2
// @description     Active Risk Control with Smart Start Strategy & Background Engine
...
(function () {
    ...
    const FALLBACK_DEFAULT_PROFILE = {
        ...
        startStrategy: { startProfile: 4, stabilityCount: 30 }
    };
    ...
    // --- STATE VARIABLES ---
    let isStartModeActive = true;
    let currentStabilityCount = 0;
    let currentRiskScore = 0; // NEW: Holds the score calculated by the background engine
            ...
            if (isStartModeActive && s.startStrategy.startProfile > 0) {
                ...
                if (hasData) {
                    ...
                    if (currentStabilityCount >= targetCount) {
                        ...
                        profileToApply = theoreticalProfile;
                    }
                }
            }
    ...
    function generateContentHtml(data) {
        // FIX: Read the score from the background engine instead of calculating it here
    ...
    async function openSettingsPanel() {
        ...
        setTimeout(() => {
            ...
            const updatePreview = () => {
                ...
                // In preview mode, we calculate based on DOM inputs, but we DO NOT call handleProfileSync
    ...
    async function init() {
        ...
        // --- NEW: BACKGROUND ENGINE LOOP ---
        // This runs independently of the UI, ensuring the Start Strategy works even if the panel is closed.
        setInterval(() => {
            const data = unsafeWindow[DATA_SOURCE];
            if (data) {
                currentRiskScore = RiskEngine.calculate(data, currentTitanSettings);
                RiskEngine.handleProfileSync(currentRiskScore, data);
            }
        }, UPDATE_INTERVAL_MS);

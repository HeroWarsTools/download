...
// @version      4.2.0
// @description  v4.2: Added "Buy Energy (50e)" feature. Checks daily limit (<2) before auto-start.
...
(function() {
    ...
    const i18n_en = {
        ...
        CAMP_TITLE: "Campaign Auto v4.2",
        CAMP_METHOD: "Method",
        CAMP_MISSION: "Mission",
        CAMP_REPS: "Reps",
        CAMP_ENERGY: "Energy >",
        CAMP_BUY_NRG: "Buy Energy (50e)",
        ...
        CAMP_STATUS_BUY: "Checking Energy (50e)...",
    ...
    const i18n_ru = {
        ...
        CAMP_TITLE: "Авто-Кампания v4.2",
        CAMP_METHOD: "Метод",
        CAMP_MISSION: "Миссия",
        CAMP_REPS: "Повторы",
        CAMP_ENERGY: "Энергия >",
        CAMP_BUY_NRG: "Купить Энергию (50e)",
        ...
        CAMP_STATUS_BUY: "Проверка Энергии (50e)...",
        ...
        const html = `
            <div id="hwh_camp_ui" style="display: flex; flex-direction: column; gap: 8px; min-width: 280px; max-width: 300px; color: #fce1ac; font-size: 13px;">
                ...
                <div style="display: flex; align-items: center; gap: 4px; font-size: 12px; margin-bottom: 4px;">
                    <input type="checkbox" id="hwh_buy_energy" ${settings.buyEnergy50 ? 'checked' : ''} onchange="HWH_CampAuto_API.autoSave()">
                    <label for="hwh_buy_energy" style="color: #00ff00;">${I18N('CAMP_BUY_NRG')}</label>
                </div>
    ...
    async function loadProfileWrapper(n) {
        ...
        if (pData) {
            ...
            setChk('hwh_buy_energy', pData.buyEnergy50 || false);
    ...
    async function runAutomationSequence() {
        ...
        // --- NEW FEATURE: BUY ENERGY 50e ---
        if (settings.buyEnergy50) {
            updateStatus(I18N('CAMP_STATUS_BUY'));
            try {
                await attemptSmartEnergyBuy();
            } catch (e) {
                console.warn("[HWH CampAuto] Buy Energy Error:", e);
            }
            // Wait 200ms as requested
    ...
    /**
     * @AI-REFERENCE: SMART ENERGY BUY LOGIC
     */
    async function attemptSmartEnergyBuy() {
        let active = true;
        let attempts = 0;
        ...
        while (active && attempts < 3) {
            attempts++;
            try {
                const userInfo = await unsafeWindow.Caller.send('userGetInfo');
                ...
                const boughtToday = energyData.boughtToday;

                // Condition: boughtToday < 2 means cost is 50 emeralds
                if (boughtToday < 2) {
                    console.log(`[HWH CampAuto] Buying Energy. Counter: ${boughtToday}. Cost: 50e.`);
                    const result = await unsafeWindow.Caller.send('refillableBuyStamina');
                    
                    if (result && result.stamina) {
                        console.log(`[HWH CampAuto] Refill successful! New stamina: ${result.stamina}`);
                        if (unsafeWindow.cheats && unsafeWindow.cheats.refreshInventory) {
                            unsafeWindow.cheats.refreshInventory();
                        }
                        // Small delay before next check in loop
                        await new Promise(resolve => setTimeout(resolve, 500));
                    } else {
                        console.warn('[HWH CampAuto] Server rejected purchase.');
                        active = false;
                    }
                } else {
                    console.log(`[HWH CampAuto] Stop Buy: Counter is ${boughtToday} (Cost > 50e).`);
                    active = false;
                }
            } catch (error) {
                console.error('[HWH CampAuto] Buy Energy Error:', error);
                active = false;
            }
        }
    }
    ...
    async function saveCurrentStateUI(profileIndex = null) {
        ...
        const uiState = {
            ...
            buyEnergy50: el('hwh_buy_energy').checked
        };

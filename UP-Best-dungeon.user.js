	...
	function init() {
		...
		unsafeWindow.HWH_Dgn_Profiles = {
			...
			async exportData() {
				...
				tx.objectStore(this.storeName).getAll().onsuccess = (e) => {
					...
					const sanitizedData = rawData.map(profile => {
						if (!profile.logic) {
							profile.logic = { scoringMethod: 'weighted', zeroDeaths: true, antiOneshot: false, deepGA: false };
						}
						return profile;
					});
			...
			readDOM() {
				...
				const getChk = (eid) => document.getElementById(eid)?.checked || false;
				const getSel = (eid) => document.getElementById(eid)?.value || 'weighted';
				...
				return {
					...
					logic: {
						scoringMethod: getSel('inp_scoringMethod'),
						zeroDeaths: getChk('inp_zeroDeaths'),
						antiOneshot: getChk('inp_antiOneshot'),
						deepGA: getChk('inp_deepGA')
					},
					tech: {
						timeoutFix: getVal('inp_timeout'), countFix: getVal('inp_countFix'),
						monoMultiplier: getVal('inp_mono_mult'),
			...
			save(id, silent = false) {
				...
				tx.oncomplete = () => {
					if (!silent) this.feedback(id, 'save', 'Saved');
				};
			},
			...
			load(id, autoApply = false) {
				...
				tx.objectStore(this.storeName).get(id).onsuccess = (e) => {
					...
					if (isPopupOpen) {
						...
						setVal('inp_scoringMethod', res.logic?.scoringMethod || 'weighted');
						setChk('inp_zeroDeaths', res.logic?.zeroDeaths !== false);
						setChk('inp_antiOneshot', res.logic?.antiOneshot || false);
						setChk('inp_deepGA', res.logic?.deepGA || false);

						setVal('inp_timeout', res.tech.timeoutFix); setVal('inp_countFix', res.tech.countFix);
						setVal('inp_mono_mult', res.tech.monoMultiplier || 1);
						...
						if (!DungeonUtils.bestParams) {
							DungeonUtils.bestParams = { populationSize: 9, generations: 30, mutationRate: 0.06, eliteCount: 2 };
						}
						...
						const newCfg = {
							...
							monoMultiplier: res.tech.monoMultiplier || 1,
							...
							scoringMethod: res.logic?.scoringMethod || 'weighted',
							zeroDeaths: res.logic?.zeroDeaths !== false,
							antiOneshot: res.logic?.antiOneshot || false,
							deepGA: res.logic?.deepGA || false
						};
						...
						DungeonUtils.logicConfig = {
							scoringMethod: newCfg.scoringMethod,
							zeroDeaths: newCfg.zeroDeaths,
							antiOneshot: newCfg.antiOneshot,
							deepGA: newCfg.deepGA
						};
			...
			buttons.testDungeon.combineList.splice(1, 0, {
				...
				onClick: async () => {
					...
					const cfg = {
						...
						monoMultiplier: savedCfg.monoMultiplier || 1,
						...
						simSamples: savedCfg.simSamples || 100, simRank: savedCfg.simRank || 85,
						hpWeight: savedCfg.hpWeight || 25, energyWeight: savedCfg.energyWeight || 1,
						scoringMethod: savedCfg.scoringMethod || 'weighted',
						zeroDeaths: savedCfg.zeroDeaths !== false,
						antiOneshot: savedCfg.antiOneshot || false,
						deepGA: savedCfg.deepGA || false
					};
					...
					if (!document.getElementById(styleId)) {
						...
						style.innerHTML = `
							...
							.hwh-chk-label { display:flex; align-items:center; cursor:pointer; margin-bottom:4px; }
							.hwh-chk-input { margin-right:8px; transform:scale(1.2); cursor:pointer; }
							.hwh-chk-text { color:#ccc; font-size:14px; }
							.hwh-native-green-btn {
							...
							.hwh-native-green-btn:active { box-shadow: inset 0px 4px 6px #1a2f04, inset 0px 4px 6px #1a2f04, inset 0px 0px 6px, 0px 0px 4px, 0px 0px 0px 1px #ce9767; }

							/* Toast Animation */
							@keyframes hwhToastFade {
					...
					const inputsTech = [
						...
						{ id: 'inp_mono_mult', label: '1elem. multiplier', val: cfg.monoMultiplier, step: 1 },
					...
					let paramsHTML = '<div style="display:flex; flex-direction:column; gap:6px; max-height: 500px; overflow-y: auto; padding-right: 5px;">';

					// Battle Logic
					paramsHTML += '<div style="color:#e6c300; font-size:16px; font-weight:bold; border-bottom:1px solid #555; margin-bottom:5px;">Battle Logic</div>';
					paramsHTML += `
						<div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:6px;">
							<label style="color:#ccc; font-size:15px;">Scoring Method</label>
							<select id="inp_scoringMethod" style="background:#222; color:#fff; border:1px solid #555; border-radius:2px; padding:4px; font-size:13px;">
								<option value="weighted" ${cfg.scoringMethod === 'weighted' ? 'selected' : ''}>Weighted (Fast)</option>
								<option value="strict" ${cfg.scoringMethod === 'strict' ? 'selected' : ''}>Strict (HP Priority)</option>
							</select>
						</div>
						<label class="hwh-chk-label" title="Discard battles where titans die (if a no-death alternative exists)">
							<input type="checkbox" id="inp_zeroDeaths" class="hwh-chk-input" ${cfg.zeroDeaths ? 'checked' : ''}>
							<span class="hwh-chk-text">Zero Deaths Priority</span>
						</label>
						<label class="hwh-chk-label" title="Prioritize battles that keep the weakest titan above 10% HP">
							<input type="checkbox" id="inp_antiOneshot" class="hwh-chk-input" ${cfg.antiOneshot ? 'checked' : ''}>
							<span class="hwh-chk-text">Anti-Oneshot (Save Weakest)</span>
						</label>
						<label class="hwh-chk-label" title="Double the simulation tests for deeper search (slower)">
							<input type="checkbox" id="inp_deepGA" class="hwh-chk-input" ${cfg.deepGA ? 'checked' : ''}>
							<span class="hwh-chk-text">Deep GA (Adaptive Search)</span>
						</label>
					`;

					// Algorithm
					paramsHTML += '<div style="color:#e6c300; font-size:16px; font-weight:bold; border-bottom:1px solid #555; margin:10px 0 5px 0;">Algorithm</div>';
					...
					// --- FULL LAYOUT WITH TOAST AND NATIVE BUTTON ---
					const fullContentHTML = `
						<div style="position: relative; padding-bottom: 10px;">
							<!-- Toast Notification -->
							<div id="hwh_save_toast" style="display:none; position:absolute; top:-10px; left:50%; transform:translateX(-50%); background:rgba(71, 164, 27, 0.9); color:#fff; padding:8px 20px; border-radius:5px; font-weight:bold; z-index:9999; border: 1px solid #1a2f04; box-shadow: 0 4px 6px rgba(0,0,0,0.5);">
								Configuration Saved!
							</div>
							...
							<!-- Native-looking Save Button at the bottom -->
							<div style="display: flex; justify-content: center; margin-top: 15px; padding-top: 15px; border-top: 1px solid #444;">
								<button id="btn_global_save" class="hwh-native-green-btn">
									💾 Save Configuration
								</button>
							</div>
						</div>`;
					...
					const saveConfigLogic = () => {
						...
						const getSel = (id) => document.getElementById(id)?.value || 'weighted';
						...
						if (unsafeWindow.HWH_Dgn_Profiles.activeProfileId) {
							unsafeWindow.HWH_Dgn_Profiles.save(unsafeWindow.HWH_Dgn_Profiles.activeProfileId, true);
						}

						console.log("HWH Ext: Configuration Applied.", newCfg);
					};
					...
					const attachListeners = setInterval(() => {
						...
						if (inpS && inpR && lbl && btnSave && toast) {
							const calc = () => {
								...
								let q = r / s;
								if (q > 0.999) q = 0.999;
								lbl.innerText = Math.floor(q * 1000) / 1000;
							};
							inpS.addEventListener('input', calc);
							inpR.addEventListener('input', calc);
							calc();

							if (!btnSave.hasAttribute('data-bound')) {
								btnSave.setAttribute('data-bound', 'true');
								btnSave.addEventListener('click', () => {
									saveConfigLogic();
									// Show Toast Animation
									toast.style.display = 'block';
									toast.style.animation = 'hwhToastFade 1.2s ease-in-out forwards';
									setTimeout(() => {
					...
					// Open Popup (Only Close button at the bottom)
					await popup.confirm(
						fullContentHTML,
						[
		...
		class BestDungeon {
			constructor(resolve, reject) {
				...
				this.monoMultiplier = savedCfg.monoMultiplier || 1;
			...
			async testProcessingPromises(battles) {
				...
				// BUGFIX: Initialize with worst possible losses [1,2,3,4,5] instead of []
				let bestRec = { hp: -Infinity, energy: -Infinity, losses: [1, 2, 3, 4, 5], minTitanHp: 0 };
				let bestPack = null;
				const allowedTitanIds = getSaveVal('allowedTitanIds', []);
		...
		class EvaluateAttackPack extends SelectAttackPack {
			...
			async getAttackers(allowedIds = []) {
				...
				let genCount = this.bestParams.generations;
				if (DungeonUtils.logicConfig && DungeonUtils.logicConfig.deepGA) {
					genCount = Math.floor(genCount * 1.5);
				}

				const ga = new GeneticAlgorithm({ values, combinationSize: 5, ...this.bestParams, generations: genCount });
